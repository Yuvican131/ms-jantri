...
export function DataEntryControls({
    ...
    const handleKeyDown = (e: React.KeyboardEvent, action?: () => void) => {
        if (e.key === 'Enter') {
          ...
          if (target.id === 'laddiNum1') laddiNum2Ref.current?.focus();
          else if (target.id === 'laddiNum2') laddiAmountRef.current?.focus();
          else if (target.id === 'laddiAmount') handleLaddiApply();
          else if (target.id === 'harupA') harupBInputRef.current?.focus();
          else if (target.id === 'harupB') harupAmountInputRef.current?.focus();
          else if (target.id === 'harupAmount') handleHarupApply();
          else if (target.tagName === 'TEXTAREA') {
              // Logic for multi-text textarea
          } else if (action) {
            action();
          }
        }
              ...
              <div className="flex items-start gap-2 mb-1">
                  <div className="flex-1 flex flex-col items-center gap-1">
                      <Input
                        ...
                        value={laddiNum1} onChange={(e) => setLaddiNum1(e.target.value.replace(/[^0-9]/g, ''))} onKeyDown={handleKeyDown} disabled={isDataEntryDisabled}
                        onClick={isDataEntryDisabled ? showClientSelectionToast : undefined}
                      />
                  </div>
                  ...
                  <div className="flex-1 flex flex-col items-center gap-1">
                      <Input
                        ...
                        value={laddiNum2} onChange={(e) => setLaddiNum2(e.target.value.replace(/[^0-9]/g, ''))} onKeyDown={handleKeyDown} disabled={isDataEntryDisabled}
                        onClick={isDataEntryDisabled ? showClientSelectionToast : undefined}
                      />
                  </div>
                  <div className="flex flex-col items-center justify-center px-1 pt-1">
            ...
            <div className="border rounded-lg p-2 flex flex-col gap-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-1">
                <div className="flex items-center gap-1">
                    ...
                    <Input ref={harupAInputRef} id="harupA" placeholder="e.g. 123" className="min-w-0 h-8 text-xs" value={harupA} onChange={(e) => setHarupA(e.target.value)} onKeyDown={(e) => handleKeyDown(e, handleHarupApply)} disabled={isDataEntryDisabled} onClick={isDataEntryDisabled ? showClientSelectionToast : undefined}/>
                </div>
                <div className="flex items-center gap-1">
                    <Label htmlFor="harupB" className="w-6 text-center shrink-0 text-xs">B</Label>
                    <Input ref={harupBInputRef} id="harupB" placeholder="e.g. 456" className="min-w-0 h-8 text-xs" value={harupB} onChange={(e) => setHarupB(e.target.value)} onKeyDown={(e) => handleKeyDown(e, handleHarupApply)} disabled={isDataEntryDisabled} onClick={isDataEntryDisabled ? showClientSelectionToast : undefined}/>
                </div>
              </div>
                <div className="flex items-center gap-2 mt-1">
                  <Label htmlFor="harupAmount" className="w-6 text-center shrink-0 text-xs">=</Label>
                  <Input ref={harupAmountInputRef} id="harupAmount" placeholder="Amount" className="font-bold h-8 text-xs" value={harupAmount} onChange={(e) => setHarupAmount(e.target.value)} onKeyDown={(e) => handleKeyDown(e, handleHarupApply)} disabled={isDataEntryDisabled} onClick={isDataEntryDisabled ? showClientSelectionToast : undefined}/>
